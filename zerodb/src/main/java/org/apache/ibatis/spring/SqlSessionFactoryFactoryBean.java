package org.apache.ibatis.spring;
...
public class SqlSessionFactoryFactoryBean implements FactoryBean,InitializingBean{
	Log log = LogFactory.getLog(SqlSessionFactoryFactoryBean.class);
	...
	private SqlSessionFactory createSqlSessionFactory() throws IOException {
		...
		try {
			...
			if(dataSource != null) {
				...
				conf.setEnvironment(new Environment("development",new JdbcTransactionFactory(),dataSourceToUse));
				sqlSessionFactory = new SqlSessionFactoryBuilder().build(conf);
			}
			
			if(mapperLocations != null) {
				Map<String, XNode> sqlFragments = conf.getSqlFragments();
				for(Resource r : mapperLocations) {
					log.info("Loading iBatis3 mapper xml from file["+r.getURL().toString()+"]");					
					InputStream in = r.getInputStream();
					try {
						XMLMapperBuilder mapperBuilder = new XMLMapperBuilder(in,conf,r.getURL().toString(),sqlFragments);
						mapperBuilder.parse();
					}finally {
						in.close();
					}
